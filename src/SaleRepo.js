...
async function SalesRankStore(cBrandNum, cUsersCde, cOtherCde, cCategNum,
    cItemDept, cItemType, cStorNum_, dDateFrom, dDateTo__) {
    ...
    try {
        ...
        if (cStorNum_) params.append('StorNum_', cStorNum_);
        ...
        if (Array.isArray(data)) {
            data.forEach(item => {
                ...
                // if (item.MaxiDate) {
                //     item.MaxiDate = formatDate(item.MaxiDate);
                // } else {
                //     console.error('MaxiDate is missing for item:', item);
                // }
            });
        }
        if (nTotalAmt !== 0) {
            ...
            <table id="salesRankTable1">
                ...
                <tbody id="rankTBody">
                    ${data.map(item => {
                        ...
                        return `
                            <tr>
                                <td class="colNoWrap">${item.StorName || 'N/A'}</td>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const closeRepo = document.querySelector('.closeRepo');
    ...
    salesRankLocationElements.forEach(element => {
        element.addEventListener('click', () => {
            ...
            SalesRankStore('00477', '', '', '', '', '', '', '10/01/2024', '10/31/2024');
        });
    });
});
