...
function getPendingMessagesForAllUsers() {
  const pendingCounts = {};
  Object.keys(PendingMessages.messages).forEach(username => {
    const counts = {};
    PendingMessages.messages[username].forEach(({ room }) => {
      const otherUser = room.split('_').find(n => n !== username);
      counts[otherUser] = (counts[otherUser] || 0) + 1;
    });
    pendingCounts[username] = counts;
  });
