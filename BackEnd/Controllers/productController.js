...
const listItem = async (req, res) => {
  const nItemPrce = req.query.ItemPrce;  
  ...
  const params = {};
  if (nItemPrce) {
    cSql += " AND ITEMLIST.ItemPrce > @nItemPrce";
    params.nItemPrce = nItemPrce;
  }
  if (cDescript) {
    cSql += " AND ITEMLIST.Descript LIKE @cDescript";
    params.cDescript = `%${cDescript}%`;  // Use wildcards for LIKE
  }
  if (cBrandNum) {
...
const editItemList = async (req, res) => {
  const { cItemCode,cUsersCde,cOtherCde,cDescript,
    ...
    nItemCost,nLandCost } = req.body;  // Extract from body
